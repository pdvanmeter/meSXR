...
class Composite_Profile(Profile):
    ...
    def domain(self, *args):
        ...
        Update: I modified behavior because it can lead to some unexpected behaviors for
        flux surface profiles that may hit the maximum radial value before the domain edge. So now
        this is the union of the individual profile domains.
        """
        return np.logical_or(self.prof1.domain(*self.prof1.transform(*args)), self.prof2.domain(*self.prof2.transform(*args)))
...
class Profile_Const(Profile_2D):
    def __init__(self, label, units='N/A', dim_units=['m', 'm'], const=1.0, boundary=0.0):
        super(Profile_Const, self).__init__(label, units=units, dim_units=dim_units, boundary=boundary)
        self.const = const

    def value(self, x, y):
        x = np.atleast_1d(x)
        return np.squeeze(np.ones(x.shape))*self.const
...
class Profile_Alpha_Beta(Profile_Radial):
    ...
    def value(self, r):
        return self.core_value*(1 - np.minimum(r,1)**self.alpha)**self.beta
...
class Profile_Peaked(Profile_Radial):
    """
    This is used to represent a profile which is peaked in the core, as expected for SHAx plasmas.
    """
    def __init__(self, label, f0, alpha, f1, delta, beta, units='N/A', dim_units=['m', 'm'], norm=MST_MINOR_RADIUS, rlim=[0., 1.]):
        super(Profile_Peaked, self).__init__(label, units=units, dim_units=dim_units, rlim=rlim, norm=norm)
        self.f0 = f0
        self.alpha = alpha
        self.f1 = f1
        self.delta = delta
    ...
    def value(self, r):
        return self.f0*(1 - np.minimum(r,1)**self.alpha) + self.f1*np.exp( -1.0*(r / self.delta)**self.beta )
...
class Profile_Peaked_Rho(Transform_Rho, Profile_Peaked):
    """
    Modification of the standard alpha-beta profile to work with the radial flux coordinate rho.
    """
    def __init__(self, label, f0, alpha, f1, delta, beta, units='N/A', dim_units=['m', 'm'], norm=MST_MINOR_RADIUS, rlim=[0., 1.],
                 delta_a=0.06, delta_h=0.01, flux=None):
        Transform_Rho.__init__(self, delta_a=delta_a, delta_h=delta_h, norm=norm, flux=flux)
        Profile_Peaked.__init__(self, label, f0, alpha, f1, delta, beta, units=units, dim_units=dim_units, norm=norm, rlim=rlim)
